...
- [ ] **4.1.1** 邮箱泄露查询 ★
  - CavalierSource 已接入（infostealer 日志）→ ✅
  - **待做**：接入 HIBP（HaveIBeenPwned）作为第二泄露数据源
  - HIBP v3 API 用 k-anonymity 模型（SHA-1 前缀匹配），不需要 API key
  - 验收：`search_by_email("test@example.com")` 返回 Cavalier + HIBP 合并结果

- [ ] **4.1.2** 邮箱反查注册平台 ★ 新增
  - **输入邮箱 → 查询该邮箱在哪些平台注册过**
  - 实现方式：
    - 方案 A：接入 holehe（200+ 平台邮箱注册检测）
    - 方案 B：接入 emailrep.io API（风险评分 + 关联平台）
    - 方案 C：从 Cavalier/HIBP 泄露记录中提取关联平台
  - 验收：`check_email("user@gmail.com")` 返回 `{platforms: ["github", "twitter", ...], risk_score: ...}`
